...
CREATE TABLE IF NOT EXISTS `acex`.`desplazamiento` (
  ...
  `empresa_transporte` VARCHAR(300) NULL,
  `importe_transporte` DOUBLE NULL,
...
CREATE TABLE IF NOT EXISTS `acex`.`grupo` (
  `id_grupo` INT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `acex`.`grupo_participa` (
  ...
  INDEX `fk_actividades_solicitadas_has_grupo_grupo1_idx` (`id_grupo` ASC) VISIBLE,
  INDEX `fk_actividades_solicitadas_has_grupo_actividades_solicitada_idx` (`id_actividad` ASC) VISIBLE,
  CONSTRAINT `fk_actividades_solicitadas_has_grupo_actividades_solicitadas2`
    FOREIGN KEY (`id_actividad`)
    REFERENCES `acex`.`actividades_solicitadas` (`id_actividad`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_actividades_solicitadas_has_grupo_grupo1`
